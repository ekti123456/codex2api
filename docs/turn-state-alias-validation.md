...
此次修改把 Codex 上游的 `X-Codex-Turn-State` 保存为加密映射，对客户端使用随机代号。日志保存代号和原值摘要，不保存官方原值。使用日志详情的“会话连续性”中可查看 `turn_state`。
...
| 数据库关闭重开、请求处理器重建 | 旧代号仍可还原，密钥保持一致 |
...
最后对代号格式检查收紧后，另行通过 `go test ./database -run '^TestTurnState' -count=1`。诊断展示测试共 11 项通过。
