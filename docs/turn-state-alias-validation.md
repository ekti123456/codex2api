...
## HTTP 响应头与回传闭环（2026-09-18）

官方 Codex HTTP/SSE 客户端从真正的 HTTP 响应头保存 turn-state，不从 SSE metadata 保存它。此前事件已替换、日志已签发，但 HTTP 客户端仍拿不到可回传的值。修复将流内首个已签发代号暂存到当前尝试的响应头，在首次实际写出前发布；原 HTTP 头优先，整次缓冲重试只发布成功尝试的头。另修复 HTTP 头独立回传后，在身份元数据快照变化时被投影整理清除的问题：校验还原后先投影到当前请求正文，WebSocket upgrade 头不参与该投影。

新增验证使用 HTTP 客户端仅从 `recorder.Result().Header` 保存代号并仅从请求头回传，覆盖 HTTP 上游、实际本地 WebSocket 上游、连接复用、429 额度切号、两种 input 处理模式及重开数据库。模拟官方同轮只保存首个值的行为：上游随后返回不同值，后续请求仍发送首个真实值；再次返回相同真实值复用映射；新轮次从空状态开始。流式、整次缓冲重试、非流式 JSON、三种 metadata 事件名以及原始值不可直接提升为响应头均有回归。

此路径不预读、不额外等待、不新增热路径数据库查询。已经提交的 HTTP 响应头不能补写（包括持续重试心跳、前置事件立即透传或 metadata 晚于正文）；这些情况下仍只替换流内代号，不伪造客户端已保存的结论。`issued` 继续表示映射签发，不是客户端确认。上游状态生成算法和字符串是否轮换不受此修复控制。
