...
The setting **向 NewAPI 上报宽松首响应 / Report loose first response to NewAPI**
replaces early metadata passthrough. Its stored/API key remains
`codex_preflight_sse_passthrough_enabled` for compatibility. It is off by default;
an existing true value now enables reports, never early writes.
...
the loose event classifier, excluding lifecycle, error, terminal and heartbeat
events. WS handshake/connection acquisition is not a response event.
