...
迁移前仍校验模型、渠道、用户分组、预算、账号容量、出口和黑名单。目标必须是不同的实际 ChatGPT 账号，并且能使用持久化账号级 ID 映射。目标与原账号的账号分组 ID 集合、标签集合必须分别完全相等：忽略顺序和重复项，不接受仅交集相同、子集或超集；未分组只匹配未分组，无标签只匹配无标签。标签忽略首尾空白，保留大小写。选号时过滤，提交迁移前再次核对；准备过程中任一方分组或标签变化则停止迁移，记录 `account_groups_changed` / `account_tags_changed`，保留原粘性。数据库以原账号及迁移代数 CAS 更新主会话和现有窗口授权；同时保留窗口创建/到期时间、序号、扩容倍率等信息，不续期、不新增免费扩容权益。未找到安全候选或校验失败即停止，不随意改号。普通请求省略窗口授权票据时，会从已验证用户的持久授权记录恢复并同步归属；过期或需重新确认的扩容授权不会被跳过。

NewAPI 的 `quote` / `quote_tiered` 在原账号普通容量满时优先保留账号：扩容已授权、个人扩容额度有剩余且原账号保留槽可用时，签发按阶梯计价的扩容预占票，即使个人普通窗口额度还有剩余也优先扩容，避免迁移丢失上下文。否则，开启自动换号且查到持久主会话归属时，将账号容量判断延后到正式请求调度，诊断标记 `capacity_failover_deferred=true`。控制请求没有完整对话和最终模型，因此不在报价时改绑或猜选目标。个人窗口限额与扩容授权仍正常计算：用户自身还有普通窗口额度时可取得普通预占票，个人限额已满仍需扩容授权。正式请求复用上述迁移及上下文清理流程；未过期且 ID 匹配的新预占票可以与主账号归属在同一事务中迁移，保留预占截止时间和价格，随后仍经正式窗口准入确认。报价成功不代表已经换号，也不保证存在合格候选。关闭自动换号、后台请求或仅有运行期归属时不使用该延期规则。已存在的有效授权沿用原价格，不在报价中静默改价；已有扩容票能在原账号准入时不触发容量换号。

候选淘汰原因会汇总到请求诊断，包括 `account_groups_mismatch`、`account_tags_mismatch` 和 `window_grant_capacity_mismatch`，不记录标签内容或凭据。
