...
候选淘汰原因会汇总到请求诊断，包括 `account_groups_mismatch` 和 `window_grant_capacity_mismatch`。候选详情可显示账号标签供管理员识别，但标签不参与筛选；不记录凭据。
