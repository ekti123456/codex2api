...
请求诊断取消原来的 12 KiB 总大小限制，完整保存已经采集的入口明细、HTTP/WS 出站身份、Turn-State 事件和工具配对诊断；数据库写入、详情读取和导出不再因总大小省略内容。采集阶段既有的白名单、单字段和来源数量边界保持不变。历史记录可能仍有 `incoming:null`、`outbound_identity.truncated=true`（界面显示 `capture_truncated=true`）；缺失或截断字段不应解读为客户端未提供或实际上游未收到该字段，已丢弃内容不能补回。
