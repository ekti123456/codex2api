...
使用统计的“请求类型”列展示每条新日志的网关判定，点击读取该日志的诊断详情。桌面表格和移动端卡片均支持，列可隐藏。详情仅开放给管理员，可以复制 JSON。认证令牌等继续脱敏；turn-state 核对值按管理员需求完整记录。

独立的 `X-Codex-Turn-State` 区域展示客户端回传值、映射/上游返回的真实值、实际出站值以及签发的代号。`turn_state.events` 的 `received`、`real`、`alias` 保存对应值，`real_hash` 保留摘要，`account_id` / `generation` 表示所属账号与切号代次。`issued` 表示已持久化签发映射（不代表失败重试中的响应已送达客户端），`restored` 表示回传代号校验通过；`cleared_*` 表示清理原因。实际出站值取自 `upstream.outbound_identity.http.headers` 和本次请求体的 `client_metadata`，不会把还原记录或复用连接的旧握手当成本次实际发送。服务错误日志保留相同的 `turn_state` 字段。

普通 292 字符值完整显示和复制；采集时单值超过 2048 字节会附加 `[truncated]` 且设置 `value_truncated`，最多采集 24 条事件。已采集的事件、比较值和出站快照完整落库，不再因诊断总大小而进一步删减。历史日志只有摘要时不能补回原值。仅收到的未知值不标为已验证真实值；其他用户/线程的映射不会因回传其代号而被披露。
...
- 入口元数据只提取一次，不反序列化整个请求正文。单个元数据对象上限 16 KiB，嵌套、来源数量和单字段采集仍有边界。取消原来的 12 KiB 总大小限制：完整保存已采集的 `incoming`、出站身份、Turn-State 事件及工具配对等诊断，写入、详情读取和导出均不按总大小删减。仅序列化一次，不增加数据库查询；较大的快照会增加相应存储及详情/导出流量，普通列表不加载这些内容。正文大小只影响定位 client_metadata 的扫描耗时，不随正文大小保留诊断副本。
- 历史记录的 `incoming:null` 与 `truncated:true` 一起出现时，界面明确提示入口快照因旧版大小限制被省略，不再逐项显示为“未提供 / 未记录”；无法恢复已被丢弃的内容。
