...
“会话连续性”部分的 `turn_state` 展示轮次状态代号的处理记录：`alias` 是发给客户端的完整代号，`real_hash` 是官方原值的摘要，`account_id` / `generation` 是所属账号与切号代次。`issued` 表示已持久化签发映射（不代表失败重试中的响应已送达客户端），`restored` 表示回传代号校验通过；`cleared_*` 表示原始旧值、未知/过期代号、作用域不匹配或账号代次变化等清理原因。官方原值不进入日志。服务错误日志保留相同的 `turn_state` 字段。
