...
- 目标账号继续满足原有分组、标签、模型、容量及窗口授权条件。没有符合条件的账号返回现有 `no_available_account`；上下文不能迁移时保留具体的上下文错误，且不发送到其他账号。
...
回归覆盖 HTTP 429、SSE 额度失败、原生 WebSocket、真实 WebSocket 上游、生命周期帧先于失败、压缩请求、临时同号限流、零预算、重试目标再次额度耗尽、关闭切号、分组/标签/模型不匹配、不可迁移上下文、API 中转豁免、首请求刚建立归属、连续迁移、重启后的归属恢复，以及切号后的工具、身份、窗口序号和 turn-state 别名。
