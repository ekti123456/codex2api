...
Codex 上游的 `X-Codex-Turn-State` 在网关入口使用持久化代号映射：客户端收到 URL-safe Base64 随机代号，外观采用 `0x80` 版本、时间、随机分块和本机签名，与已观察到的 `gAAAA…` 格式相似。原值符合该封装时保留长度及公开的版本/时间部分，其余内容重新随机生成；其他原值使用默认 292 字符代号。这不生成有效的官方密文。回传时先校验本机签名，再按用户、原始主会话、线程/轮次、上游账号及切号代次校验映射，通过后才还原官方原值。HTTP 响应头和 SSE/WebSocket 的 `response.metadata.headers` 使用相同映射。上游没返回该字段时不会补造。不兼容旧 `c2ts_v1_...` 代号，旧记录也不再复用。

HTTP `/v1/responses` 会把当前尝试流内首个已映射的 turn-state 同步到真正的 HTTP 响应头，原有上游 HTTP 头优先；同一次响应中的后续 metadata 不覆盖它。普通流在首次下发正文前提交，整次缓冲重试只提交最终成功尝试的头。客户端仅通过请求头回传时，还原后的值也会投影到当前请求的 `client_metadata`，确保身份改写及 HTTP→WebSocket 转换不会丢失；WebSocket 握手中的旧值不会投影到后续帧。若心跳、显式前置事件透传或正文已经提交 HTTP 响应头，迟到的 metadata 无法再补入该头；仍会替换流内值，但不把“签发”视为客户端已保存。此处理不额外预读整个响应，不增加等待窗口。

切号后旧代号会清除，不改绑到新账号，也不据此重新选择旧账号；即便客户端在同一轮一直回传第一次收到的代号，也不会把旧账号的状态发送给新账号。未知、过期、不匹配以及升级前未受管的原始状态均按无状态处理。账号切回原账号时仍校验切号代次。此映射不替代现有会话归属和连续性校验。

映射有效期为 7 天，活跃映射临近过期会续期；真实值使用 AES-GCM 加密保存，密钥随数据库持久化。重启和共用同一数据库的实例可继续使用，备份需包含 `codex_turn_state_secret` 和 `codex_turn_states`。密钥与密文在同一数据库，这不是抵御完整数据库泄露的独立密钥托管方案。过期记录按索引分批清理，请求内复用已查询/签发结果，普通文本流片段不查询数据库。

管理员使用日志诊断和服务错误日志的 `turn_state` 中，`received` 记录客户端回传值，`real` 记录映射或上游返回的真实值，`alias` 记录代号，`real_hash` 保留摘要。`account_id` / `generation` 标明账号和切号代次，`action` 区分签发、还原和清除原因。真实值和代号通常完整显示，单值超过 2048 字节或总诊断超出预算时会截断并标记；旧摘要无法还原原值。未知客户端值仅记为收到值，不冒充已验证的真实值；跨作用域拒绝时不泄露他人映射。实际出站头或元数据另在 `upstream.outbound_identity` 中记录。映射表仍加密，但管理员日志包含明文核对值。映射保存失败会停止该次转发，不回退为原值透传。
