...
Codex 上游的 `X-Codex-Turn-State` 在网关入口使用持久化代号映射：客户端只收到 `c2ts_v1_...` 随机代号，回传时按用户、原始主会话、线程/轮次、上游账号及切号代次校验，通过后才还原官方原值。HTTP 响应头和 SSE/WebSocket 的 `response.metadata.headers` 使用相同映射。网关不生成官方状态，上游没返回该字段时不会补造。
...
使用日志诊断和服务错误日志新增 `turn_state`：`alias` 记录客户端代号，`real_hash` 记录官方原值摘要，`account_id` / `generation` 标明账号和切号代次，`action` 区分签发、还原和清除原因。日志不记录官方原值。映射保存失败会停止该次转发，不回退为原值透传。
