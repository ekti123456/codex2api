...
func TestCodexSessionPreserveInputSettingsSQLiteDefaultsAndMigration(test *testing.T) {
	...
	if err != nil || settings == nil || settings.CodexSessionFailoverPreserveInput {
		test.Fatalf("fresh setting must default off: settings=%+v err=%v", settings, err)
	}
	if _, err := db.conn.ExecContext(ctx, "UPDATE system_settings SET codex_session_failover_preserve_input = NULL WHERE id = 1"); err != nil {
		test.Fatal(err)
	}
	settings, err = db.GetSystemSettings(ctx)
	if err != nil || settings == nil || settings.CodexSessionFailoverPreserveInput {
	...
	settings.CodexSessionFailoverPreserveInput = true
	if err := db.UpdateSystemSettings(ctx, settings); err != nil {
		test.Fatal(err)
	}
	if _, err := db.conn.ExecContext(ctx, "ALTER TABLE system_settings DROP COLUMN codex_session_failover_preserve_input"); err != nil {
		test.Fatal(err)
	}
	...
	if settings.CodexSessionFailoverPreserveInput || settings.SiteName != "existing installation" || settings.CodexImagesMainModel != "gpt-5.6-luna" || !settings.CodexTelemetryEnabled {
		test.Fatal("migration enabled failover or changed existing settings")
	}
	testCodexSessionPreserveInputSettingsRoundTrip(test, db)
}

func TestCodexSessionPreserveInputSettingsPostgres(test *testing.T) {
...
func testCodexSessionPreserveInputSettingsRoundTrip(test *testing.T, db *DB) {
	test.Helper()
	ctx := context.Background()
			...
			for _, preserveKey := range []bool{true, false} {
				test.Run(fmt.Sprintf("enabled=%t/patterns=%t/key=%t", enabled, preservePatterns, preserveKey), func(test *testing.T) {
					settings := &SystemSettings{
						CodexSessionFailoverPreserveInput: !enabled,
					...
					settings.CodexSessionFailoverPreserveInput = enabled
					...
					if persisted.CodexSessionFailoverPreserveInput != enabled || persisted.CodexImagesMainModel != settings.CodexImagesMainModel || !persisted.CodexTelemetryEnabled {
