...
func (h *Handler) ExportUsageLogs(c *gin.Context) {
	...
	case "filtered":
		...
		filter = &parsed
		for _, key := range []string{"start", "end", "q", "search_scope", "request_type", "turn_state", "turn_state_length", "model", "endpoint", "api_key_id", "account_id", "fast", "stream", "compact", "has_compaction_history", "channel", "status", "status_code", "error_only", "error_kind", "retry", "via_websocket", "include_canceled", "email", "request_id", "upstream_request_id"} {
