...
func TestCodexSessionPreserveInputSettingsRoundTrip(test *testing.T) {
	...
	if settings.CodexSessionFailoverPreserveInput {
	...
	for _, step := range []struct {
		...
		{name: "enable", patch: `{"codex_session_failover_preserve_input":true}`, want: true, status: http.StatusOK},
		...
		{name: "null is omitted", patch: `{"codex_session_failover_preserve_input":null}`, want: true, status: http.StatusOK},
		{name: "invalid boolean", patch: `{"codex_session_failover_preserve_input":"true"}`, want: true, status: http.StatusBadRequest},
		{name: "disable", patch: `{"codex_session_failover_preserve_input":false}`, status: http.StatusOK},
		...
		test.Run(step.name, func(test *testing.T) {
			if step.stale {
				proxy.UpdateRuntimeSettings(func(current proxy.RuntimeSettings) proxy.RuntimeSettings {
					current.CodexSessionFailoverPreserveInput = !step.want
					return current
				})
			}
			...
			if step.status == http.StatusOK {
				var response struct {
					Enabled *bool `json:"codex_session_failover_preserve_input"`
				}
			...
			if proxy.CurrentRuntimeSettings().CodexSessionFailoverPreserveInput != step.want {
			...
			if persisted.CodexSessionFailoverPreserveInput != step.want || persisted.CodexCapacityRetryEnabled != settings.CodexCapacityRetryEnabled || persisted.CodexOverloadPauseEnabled != settings.CodexOverloadPauseEnabled {
