...
func TestOutboundIdentityHeadersPrivacyAndBounds(test *testing.T) {
	...
	headers.Set("X-Codex-Turn-State", "turn-state-for-admin-comparison")
	...
	require.Equal(test, "turn-state-for-admin-comparison", captured.Headers["X-Codex-Turn-State"])
...
func TestOutboundIdentityOversizeDoesNotEraseRequestDiagnostic(test *testing.T) {
	...
	require.Greater(test, len(usage.RequestDiagnostics), 12*1024)
	require.Equal(test, "user", gjson.Get(usage.RequestDiagnostics, "resolved.thread_source").String())
	require.False(test, gjson.Get(usage.RequestDiagnostics, "truncated").Bool())
	require.False(test, gjson.Get(usage.RequestDiagnostics, "upstream.outbound_identity.truncated").Bool())
	identityJSON, err := json.Marshal(snapshotUpstreamTrace(request.Request.Context()).Transport.OutboundIdentity)
	require.NoError(test, err)
	require.JSONEq(test, string(identityJSON), gjson.Get(usage.RequestDiagnostics, "upstream.outbound_identity").Raw)
