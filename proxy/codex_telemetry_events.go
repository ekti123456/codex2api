...
func buildCodexTelemetryProfile(client codexTelemetryClient, input codexTelemetryRequest) codexTelemetryProfile {
	...
	sessionID := firstNonEmptyString(metadata.Get("session_id").String(), gjson.GetBytes(input.body, "client_metadata.session_id").String(), input.headers.Get(codexSessionIDHeader), input.headers.Get(codexLegacySessionIDHeader), input.sessionID)
	threadID := firstNonEmptyString(metadata.Get("thread_id").String(), gjson.GetBytes(input.body, "client_metadata.thread_id").String(), input.headers.Get(codexThreadIDHeader), sessionID)
	if sessionID == "" {
